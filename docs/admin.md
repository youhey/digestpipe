...
List page の `Export Excel` / `Import Excel` action から、Feed Sources master data を XLSX で入出力できます。列は `key`, `name`, `url`, `language`, `enabled`, `analysis_enabled`, `tier`, `category`, `sort_order` です。Import は `key` を自然キーとして作成または更新します。Upload した XLSX は import 処理にだけ使い、master data としてファイル自体は保存しません。
...
API Tokens resource は `/admin/api-tokens` にあります。Settings group から開けます。
...
Positive Keywords と Negative Keywords の List page には、それぞれ `Export Excel` / `Import Excel` action があります。列は `keyword`, `score`, `enabled`, `locale`, `category`, `notes`, `sort_order`, `match_mode` です。Import は Resource に応じて `type=positive` または `type=negative` を自動設定し、`type + keyword` を自然キーとして作成または更新します。Upload した XLSX は import 処理にだけ使い、master data としてファイル自体は保存しません。
