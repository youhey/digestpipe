...
digestpipe が DB の `news_items` table で保存・管理する処理単位です。

実装上は `NewsItem` モデルです。
Feed Item 由来の情報、本文取得状態、選考状態、分析状態と Analysis JSON を同一レコードで管理します。

現行実装では `NewsItem` / `news_items` として扱っています。  
将来的に `DigestItem` / `digest_items` へ改名する可能性があります。
...
`FetchNewsItemArticleContentJob` が HTML を取得します。
...
```txt
FetchNewsItemArticleContentJob
```
...
実装上は `NewsItem::readyForDigest()` と `NewsItem::hasCompletedAnalysis()` が同じ条件を見ます。
