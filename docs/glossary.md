...
`feed_sources` table に保存される RSS / Atom feed の設定です。

Feed Source は DB-backed master data で、Filament admin panel から編集できます。初期値は database seeder により登録されます。
...
Feed Source の runtime は DB から `sort_order ASC`, `id ASC` の順で読み込みます。`feed_sources` は `src/config/digestpipe.php` には定義しません。
