        ...
        return [
                ...
                ->action(static function (MasterDataSpreadsheetService $spreadsheets): StreamedResponse {
                    return response()->streamDownload(
                        static function () use ($spreadsheets): void {
                            echo $spreadsheets->exportSelectionKeywords('positive');
                        },
                        'positive-keywords.xlsx',
            ...
            Action::make('importExcel')
                ...
                ->action(function (array $data, MasterDataSpreadsheetService $spreadsheets): void {
                    try {
                        $result = $spreadsheets->importSelectionKeywords($this->uploadedPath($data['file'] ?? null), 'positive');
                    } catch (Throwable $exception) {
                        Notification::make()
                            ->danger()
                            ->title('Positive Keywords を import できませんでした。')
                    ...
                    Notification::make()
                        ->success()
                        ->title('Positive Keywords を import しました。')
                        ->body($result->summary())
                        ->send();
                }),
            CreateAction::make()
